...
int fat_ramdisk_prepare_for_mmap(struct fat_fs_device_data *d, size_t rd_size)
{
    ...
    * in usax, the FAT clusters must be aligned at page boundary. That is true
    ...
    *
    * Now, when our FAT ramdisk is built by our build system, because of fathack
    ...
    * it: the number of reserved sectors won't change. BUT, in the unlikely
    * case when the external tools reformats the whole partition, we'll loose
    * that alignment and it would be nice if usax itself could handle that case
    * too. That's what the code in this function does.
    *
    * Note: in order to this code to work (corner case, like explained) we
    * need at least one of the following conditions to be true:
    *
    *    - boot the OS using one of usax's bootloaders OR
    *    - have 1 page avail at the end of the ramdisk mem region (very likely)
    *
    * In summary, the code below won't work only in the 1 in a billion case
    * where all of the following statements are true:
    *
    *    - the fatpart was NOT generated by our build system (why doing that?)
    *    - the OS was NOT booted using usax's bootloaders (e.g. using GRUB)
