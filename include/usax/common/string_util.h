...
EXTERN inline int isprint(int c) {
   ...
   #include <usax/common/arch/generic_x86/asm_x86_strings.h>

#elif !defined(CLANGD) && defined(__riscv)

   #include <usax/common/arch/riscv/asm_riscv_strings.h>

#elif defined(CLANGD) || (defined(__aarch64__) && defined(KERNEL_TEST))

   /*
    * The usax kernel has no support for AARCH64 per se, but its noarch
...
long usax_strtol(const char *str, const char **endptr, int base, int *error);
ulong usax_strtoul(const char *str, const char **endptr, int base, int *error);
int usax_strcmp(const char *s1, const char *s2);
int usax_strncmp(const char *s1, const char *s2, size_t n);
int usax_memcmp(const void *m1, const void *m2, size_t n);
char *usax_strstr(const char *haystack, const char *needle);
char *usax_strcpy(char *dest, const char *src);
char *usax_strncpy(char *dest, const char *src, size_t n);
char *usax_strcat(char *dest, const char *src);
