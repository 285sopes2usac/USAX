...
#define usax_CMD_SYSCALL    499

enum usax_cmd {

   usax_CMD_RUN_SELFTEST        = 0,
   usax_CMD_GCOV_GET_NUM_FILES  = 1,
   usax_CMD_GCOV_FILE_INFO      = 2,
   usax_CMD_GCOV_GET_FILE       = 3,
   usax_CMD_QEMU_POWEROFF       = 4,
   usax_CMD_SET_SAT_ENABLED     = 5,
   usax_CMD_DEBUG_PANEL         = 6,
   usax_CMD_TRACING_TOOL        = 7,
   usax_CMD_PS_TOOL             = 8,
   usax_CMD_DEBUGGER_TOOL       = 9,
   usax_CMD_CALL_FUNC_0         = 10,
   usax_CMD_GET_VAR_LONG        = 11,
   usax_CMD_BUSY_WAIT           = 12,
