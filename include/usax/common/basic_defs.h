...
#include <sys/types.h>    // system header (just for ulong)
   ...
   /*
    * The usax kernel doesn't support ARM or AARCH64 (yet), but it can be
...
/*
 * usax's off_t, unrelated with any external files.
 *
 * The size of `offt` determine the max size of files and devices that usax
 * can access. By default, it's a signed 64-bit integer and that doesn't have
 * a measurable impact on usax's performance even on 32-bit systems where
 ...
 *
 * NOTE: usax support only Clang's -Wconversion, not GCC's.
 */
